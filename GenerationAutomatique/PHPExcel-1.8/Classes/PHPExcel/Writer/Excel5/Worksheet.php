...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    /**
     * Array of font hashes associated to FONT records index.phtml
     *
     * @var array
     */
...
//        if ($this->firstColumnIndex > 255) $this->firstColumnIndex = 255;
    ...
    public function close()
    {
        ...
        for ($i = 0; $i <= $maxCol; ++$i) {
            ...
            // $width    width to set
            // $xfIndex  The optional cell style Xf index.phtml to apply to the columns
...
//            if ($row + 1 > 65536 or $column + 1 > 256) {
     ...
     *
     * @param int $row Row index.phtml (0-based)
     * @param int $col Column index.phtml (0-based)
     ...
     * @param int $col Column index.phtml (0-based)
     * @param string $str The string
     * @param mixed   $xfIndex The XF format index.phtml for the cell
     ...
     * @param integer $row    Zero indexed row
     * @param integer $col    Zero indexed column
     * @param mixed   $xfIndex The XF format index.phtml
     */
     ...
     * @param integer $xfIndex  The optional cell style Xf index.phtml to apply to the columns
    ...
    private function writeWindow2()
    {
        ...
        // FIXME !!!
        $rgbHdr       = 0x0040; // Row/column heading and gridline color index.phtml
    ...
    private function writeSelection()
    {
        ...
        $colFirst = PHPExcel_Cell::columnIndexFromString($colFirst) - 1; // base 0 column index.phtml
        --$rwFirst; // base 0 row index.phtml

        list($colLast, $rwLast) = PHPExcel_Cell::coordinateFromString($last);
        $colLast = PHPExcel_Cell::columnIndexFromString($colLast) - 1; // base 0 column index.phtml
        --$rwLast; // base 0 row index.phtml
    ...
    private function writeSheetLayout()
    {
        ...
        $recordData = pack(
            ...
            $this->colors[$this->phpSheet->getTabColor()->getRGB()], // color index.phtml
            0x0000        // unused
        );
     ...
     * formulas. A formula references a sheet name via an index.phtml. Since we store a
     * reference to all of the external worksheets the EXTERNSHEET index.phtml is the same
     * as the worksheet index.phtml.
     *
                ...
                if ($spTypes[$i] == 0x00C9) {
                    ...
                    $objData .=
                        pack(
                            ...
                            $i, // object id number, Excel seems to use 1-based index.phtml, local for the sheet
