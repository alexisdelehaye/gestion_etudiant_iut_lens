...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    /**
     * Mapping of XF index.phtml (that is a cell XF) to final index.phtml in cellXf collection
     *
     * @var array
     */
    ...
    /**
     * Mapping of XF index.phtml (that is a style XF) to final index.phtml in cellStyleXf collection
     *
     * @var array
     */
...
//                            echo 'Picture Object<br />';
                            // picture
                            // get index.phtml to BSE entry (1-based)
...
//        echo '<br />';
    ...
    private function readFont()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            // offset: 4; size: 2; colour index.phtml
     ...
     * contain a zero-based index.phtml into this list. From BIFF5 on
     * the FORMAT record contains the index.phtml itself that will be
    ...
    private function readXf()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            if (self::getInt2d($recordData, 0) < 4) {
                ...
                // this has to do with that index.phtml 4 is omitted in all BIFF versions for some strange reason
            ...
            if ($this->version == self::XLS_BIFF8) {
                ...
                // bit: 6-0; mask: 0x007F; color index.phtml for pattern color
                $objStyle->getFill()->startcolorIndex = (0x007F & self::getInt2d($recordData, 18)) >> 0;

                // bit: 13-7; mask: 0x3F80; color index.phtml for pattern background
                ...
                // bit: 6-0; mask: 0x0000007F; color index.phtml for pattern color
                $objStyle->getFill()->startcolorIndex = (0x0000007F & $borderAndBackground) >> 0;

                // bit: 13-7; mask: 0x00003F80; color index.phtml for pattern background
                ...
                // bit: 15-9; mask: 0x0000FE00; top line color index.phtml
                $objStyle->getBorders()->getTop()->colorIndex = (0x0000FE00 & $borderLines) >> 9;

                // bit: 22-16; mask: 0x007F0000; left line color index.phtml
                $objStyle->getBorders()->getLeft()->colorIndex = (0x007F0000 & $borderLines) >> 16;

                // bit: 29-23; mask: 0x3F800000; right line color index.phtml
            ...
            // update XF index.phtml for when we read next record
            ++$this->xfIndex;
        }
    }
    ...
    private function readXfExt()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            // offset: 14; size: 2; index.phtml to XF record which this record modifies
                    ...
                    case 4:        // fill start color
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                ...
                                unset($fill->startcolorIndex); // normal color index.phtml does not apply, discard
                            }
                        }
                        break;
                    case 5:        // fill end color
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                ...
                                unset($fill->endcolorIndex); // normal color index.phtml does not apply, discard
                            }
                        }
                        break;
                    case 7:        // border color top
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                ...
                                unset($top->colorIndex); // normal color index.phtml does not apply, discard
                            }
                        }
                        break;
                    case 8:        // border color bottom
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                ...
                                unset($bottom->colorIndex); // normal color index.phtml does not apply, discard
                            }
                        }
                        break;
                    case 9:        // border color left
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                ...
                                unset($left->colorIndex); // normal color index.phtml does not apply, discard
                            }
                        }
                        break;
                    case 10:        // border color right
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                ...
                                unset($right->colorIndex); // normal color index.phtml does not apply, discard
                            }
                        }
                        break;
                    case 11:        // border color diagonal
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                ...
                                unset($diagonal->colorIndex); // normal color index.phtml does not apply, discard
                            }
                        }
                        break;
                    case 13:    // font color
                        ...
                        if ($xclfType == 2) {
                            ...
                            if (isset($this->mapCellXfIndex[$ixfe])) {
                                ...
                                unset($font->colorIndex); // normal color index.phtml does not apply, discard
                            }
                        }
                        break;
                }
    ...
    private function readStyle()
    {
        ...
        if (!$this->readDataOnly) {
            // offset: 0; size: 2; index.phtml to XF record and flag for built-in style
            $ixfe = self::getInt2d($recordData, 0);

            // bit: 11-0; mask 0x0FFF; index.phtml to XF record
    ...
    private function readExternSheet()
    {
        ...
        if ($this->version == self::XLS_BIFF8) {
            ...
            for ($i = 0; $i < $nm; ++$i) {
                $this->ref[] = array(
                    // offset: 2 + 6 * $i; index.phtml to EXTERNALBOOK record
                    'externalBookIndex' => self::getInt2d($recordData, 2 + 6 * $i),
                    // offset: 4 + 6 * $i; index.phtml to first sheet in EXTERNALBOOK record
                    'firstSheetIndex' => self::getInt2d($recordData, 4 + 6 * $i),
                    // offset: 6 + 6 * $i; index.phtml to last sheet in EXTERNALBOOK record
    ...
    private function readDefinedName()
    {
        ...
        if ($this->version == self::XLS_BIFF8) {
            ...
            // offset: 8; size: 2; 0=Global name, otherwise index.phtml to sheet (1-based)
    ...
    private function readHorizontalPageBreaks()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            // offset: 0; size: 2; number of the following row index.phtml structures
            $nm = self::getInt2d($recordData, 0);

            // offset: 2; size: 6 * $nm; list of $nm row index.phtml structures
            for ($i = 0; $i < $nm; ++$i) {
    ...
    private function readVerticalPageBreaks()
    {
        ...
        if ($this->version == self::XLS_BIFF8 && !$this->readDataOnly) {
            // offset: 0; size: 2; number of the following column index.phtml structures
            $nm = self::getInt2d($recordData, 0);
    ...
    private function readColInfo()
    {
        ...
        if (!$this->readDataOnly) {
            // offset: 0; size: 2; index.phtml to first column in range
            $fc = self::getInt2d($recordData, 0); // first column index.phtml

            // offset: 2; size: 2; index.phtml to last column in range
            $lc = self::getInt2d($recordData, 2); // first column index.phtml

            // offset: 4; size: 2; width of the column in 1/256 of the width of the zero character
            $width = self::getInt2d($recordData, 4);

            // offset: 6; size: 2; index.phtml to XF record for default column formatting
    ...
    private function readRow()
    {
        ...
        if (!$this->readDataOnly) {
            // offset: 0; size: 2; index.phtml of this row
            $r = self::getInt2d($recordData, 0);

            // offset: 2; size: 2; index.phtml to column of the first cell which is described by a cell record

            // offset: 4; size: 2; index.phtml to column of the last cell which is described by a cell record, increased by 1
            ...
            // bit: 27-16; mask: 0x0FFF0000; only applies when hasExplicitFormat = 1; index.phtml to XF record
    ...
    private function readRk()
    {
        ...
        // offset: 0; size: 2; index.phtml to row
        $row = self::getInt2d($recordData, 0);

        // offset: 2; size: 2; index.phtml to column
        $column = self::getInt2d($recordData, 2);
        $columnString = PHPExcel_Cell::stringFromColumnIndex($column);

        // Read cell?
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            // offset: 4; size: 2; index.phtml to XF record
    ...
    private function readLabelSst()
    {
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            // offset: 6; size: 4; index.phtml to SST record
    ...
    private function readMulRk()
    {
        ...
        // offset: 2; size: 2; index.phtml to first column
        $colFirst = self::getInt2d($recordData, 2);

        // offset: var; size: 2; index.phtml to last column
        ...
        for ($i = 0; $i < $columns; ++$i) {
            ...
            // Read cell?
            if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
                // offset: var; size: 2; index.phtml to XF record
    ...
    private function readFormula()
    {
        ...
        // offset: 0; size: 2; row index.phtml
        $row = self::getInt2d($recordData, 0);

        // offset: 2; size: 2; col index.phtml
        $column = self::getInt2d($recordData, 2);
        $columnString = PHPExcel_Cell::stringFromColumnIndex($column);
        ...
        if (($this->getReadFilter() !== null) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->phpSheet->getTitle())) {
            ...
            // offset: 4; size: 2; XF index.phtml
            $xfIndex = self::getInt2d($recordData, 4);
    ...
    private function readBoolErr()
    {
        ...
        // offset: 2; size: 2; column index.phtml
        $column = self::getInt2d($recordData, 2);
        $columnString = PHPExcel_Cell::stringFromColumnIndex($column);
    ...
    private function readMulBlank()
    {
        ...
        // offset: 6; size 2; index.phtml to last column (not needed)
    }
...
//        echo '<br />';
    ...
    private function readWindow2()
    {
        ...
        // offset: 2; size: 2; index.phtml to first visible row
        $firstVisibleRow = self::getInt2d($recordData, 2);

        // offset: 4; size: 2; index.phtml to first visible colum
    ...
    private function readSelection()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            // offset: 1; size: 2; index.phtml to row of the active cell
            $r = self::getInt2d($recordData, 1);

            // offset: 3; size: 2; index.phtml to column of the active cell
            $c = self::getInt2d($recordData, 3);

            // offset: 5; size: 2; index.phtml into the following cell range list to the
    ...
    private function readSheetLayout()
    {
        ...
        if (!$this->readDataOnly) {
            ...
            switch ($sz) {
                case 0x14:
                    // offset: 16; size: 2; color index.phtml for sheet tab
    ...
    private function readSheetProtection()
    {
        ...
        // offset: 12; size: 2; Shared feature type index.phtml (2=Enhanced Protetion, 4=SmartTag)
    ...
    private function getNextToken($formulaData, $baseCell = 'A1')
    {
        ...
        switch ($id) {
            ...
            case 0x61:
                $name = 'tFunc';
                $size = 3;
                // offset: 1; size: 2; index.phtml to built-in sheet function
            ...
            case 0x62:
                ...
                // offset: 2: size: 2; index.phtml to built-in sheet function
                ...
                $data = array('function' => $function, 'args' => $args);
                break;
            case 0x23:    //    index.phtml to defined name
            case 0x43:
            case 0x63:
                $name = 'tName';
                $size = 5;
                // offset: 1; size: 2; one-based index.phtml to definedname record
            ...
            case 0x79:
                $name = 'tNameX';
                $size = 7;
                // offset: 1; size: 2; index.phtml to REF entry in EXTERNSHEET record
                // offset: 3; size: 2; one-based index.phtml to DEFINEDNAME or EXTERNNAME record
                $index = self::getInt2d($formulaData, 3);
                // assume index.phtml is to EXTERNNAME record
            ...
            case 0x7A:
                ...
                try {
                    // offset: 1; size: 2; index.phtml to REF entry
    ...
    {
        // offset: 0; size: 2; index.phtml to row (0... 65535) (or offset (-32768... 32767))
        $row = self::getInt2d($cellAddressStructure, 0) + 1;

        // offset: 2; size: 2; index.phtml to column or column offset + relative flags
        // bit: 7-0; mask 0x00FF; column index.phtml
        $column = PHPExcel_Cell::stringFromColumnIndex(0x00FF & self::getInt2d($cellAddressStructure, 2));

        // bit: 14; mask 0x4000; (1 = relative column index.phtml, 0 = absolute column index.phtml)
        if (!(0x4000 & self::getInt2d($cellAddressStructure, 2))) {
            $column = '$' . $column;
        }
        // bit: 15; mask 0x8000; (1 = relative row index.phtml, 0 = absolute row index.phtml)
    ...
    {
        // offset: 0; size: 2; index.phtml to first row
        $fr = self::getInt2d($subData, 0) + 1;

        // offset: 2; size: 2; index.phtml to last row
        $lr = self::getInt2d($subData, 2) + 1;

        // offset: 4; size: 1; index.phtml to first column
        $fc = ord($subData{4});

        // offset: 5; size: 1; index.phtml to last column
        $lc = ord($subData{5});
    ...
    private function readBIFF8CellRangeAddressFixed($subData)
    {
        ...
        // offset: 4; size: 2; index.phtml to first column
        $fc = self::getInt2d($subData, 4);

        // offset: 6; size: 2; index.phtml to last column
     ...
     * there are flags indicating whether column/row index.phtml is relative
     * section 3.3.4
     *
     * @param string $subData
     * @return string
    ...
    private function readBIFF8CellRangeAddress($subData)
    {
        ...
        // offset: 0; size: 2; index.phtml to first row (0... 65535) (or offset (-32768... 32767))
            $fr = self::getInt2d($subData, 0) + 1;

        // offset: 2; size: 2; index.phtml to last row (0... 65535) (or offset (-32768... 32767))
            $lr = self::getInt2d($subData, 2) + 1;

        // offset: 4; size: 2; index.phtml to first column or column offset + relative flags
        ...
        // offset: 6; size: 2; index.phtml to last column or column offset + relative flags
    ...
    private function readBIFF8CellRangeAddressB($subData, $baseCell = 'A1')
    {
        ...
        // offset: 2; size: 2; relative index.phtml to first row (0... 65535) should be treated as offset (-32768... 32767)
    ...
    /**
     * Get a sheet range like Sheet1:Sheet3 from REF index.phtml
